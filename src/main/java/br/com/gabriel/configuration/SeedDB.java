...
@SpringBootApplication(scanBasePackages = {"br.com.gabriel."})
public class SeedDB {
	...
	InitializingBean seed() {
		return () -> {
			...
			PokemonListResponse plr = restTemplate.getForObject("https://pokeapi.co/api/v2/pokemon?limit=10",
			...
			plr.results.forEach(p -> {
				...
				PokemonEntity pokemon = new PokemonEntity(name, imagefront, imageback, 0);
				service.create(new PokemonDTO(pokemon));
			});
		};
	}
